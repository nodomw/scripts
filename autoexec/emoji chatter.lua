--[[
- [x] Implement caching the emojis file list, possibly find a more complete and better list.
...
if not isfile("chatter_emojis.json") then
	...
	emojil = game:GetService("HttpService"):JSONDecode(readfile("chatter_emojis.json"))
else
	emojil = game:GetService("HttpService"):JSONDecode(readfile("chatter_emojis.json"))
end
local emotes = {}
...
if not isfile("unicode.lua") then
	...
	unicode = loadstring(readfile("unicode.lua"))()
else
	unicode = loadstring(readfile("unicode.lua"))()
end
